...
uniform float flow_strength : hint_range(0.0, 1.0);
...
uniform float highlight_size : hint_range(0.0, 1.0);
uniform float highlight_strength : hint_range(0.0, 1.0);
uniform float shadow_size : hint_range(0.0, 1.0);
...
vec4 getFlow(vec4 flow_uv, float time) {	
	...
	return dual_flow * flow_strength * time;
}
...
vec3 getNormal(vec2 coord, float wave_density_1, float wave_density_2, vec4 flow, float time) {
	...
	// adding two vectors with xyz all ranging 0 to 1 results in a vector range 0 to 2
	// subtract one to make them all range -1 to -1
	avg -= 1.0;
...
void fragment () {
	...
	if (c.a > 0.0) {
		...
		vec2 tex_sample_offset = normal.xy;
